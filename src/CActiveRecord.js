...
class CActiveRecord {
    ...
    async commit(){
        ...
        const insertNewRecord = async()=>{
            ...
            const sql="INSERT INTO `"+this.tableName+"` ("+fields.join(",")+") VALUES ("+values.join(",")+")";
            if(this.debug) {
                console.log([sql,params]);
            }
        ...
        const updateRecord = async()=>{
            ...
            const sql="UPDATE `"+this.tableName+"` SET "+setValues.join(", ")+" WHERE "+conditions.join(" AND ")+" LIMIT 1";
            if(this.debug) {
                console.log([sql,params]);
            }
            return this.db.execute(sql,params);
