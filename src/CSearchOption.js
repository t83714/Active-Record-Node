...
class CSearchOption {
    ...
    [proxyHandlerSymbols.set](property, value) {
        if (supportOperators.indexOf(property) === -1)
            throw new Error(`Unsupport operator ${property}`);
    ...
    getSQLConditions() {
        ...
        const createInConditionParams = opt => {
            ...
            if (typeof value !== "string" && !Array.isArray(value))
                throw new Error(
                    "Parameter value for IN operation should be either string or Array"
                );
            ...
            if (value.length)
                throw new Error(
                    "Parameter value for IN operation should include at leaset one item"
                );
        ...
        Object.keys(this.operators).forEach(opt => {
            switch (opt) {
                case "=":
                    if (this.operators[opt] === null)
                        conditions.push(`\`${this.fieldName}\` IS NULL`);
                    else {
                ...
                case "!=":
                    if (this.operators[opt] === null)
                        conditions.push(`\`${this.fieldName}\` IS NOT NULL`);
                    else {
                        createStdConditionParams(opt);
                    }
                    break;
        ...
        return [conditions.join(this.seperator), params];
    }
}
