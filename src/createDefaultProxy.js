...
const _isPublicProp = (target, prop) => {
    if (prop === "constructor" && typeof target[prop] === "function")
        return false;
...
const createDefaultProxy = function(
    cls,
    proxyDef = {},
    exposePublicProperties = true
) {
    return function(...arg) {
        this._obj = new (Function.prototype.bind.apply(
            cls,
            [this].concat(arg)
        ))();
        ...
        publicMethodList = Array.isArray(proxyDef.publicMethodList)
            ? proxyDef.publicMethodList
            : null;
        publicPropList = Array.isArray(proxyDef.publicPropList)
            ? proxyDef.publicPropList
            : null;

        if (publicPropList && publicMethodList) {
            publicPropList = uniq(publicPropList.concat(publicMethodList));
        }

        if (typeof proxyDef.isPublicMethod === "function")
            isPublicMethod = proxyDef.isPublicMethod;
        else if (Array.isArray(publicMethodList))
            isPublicMethod = _isPublicPropByList.bind(this, publicMethodList);
        else isPublicMethod = _isPublicMethod;

        if (typeof proxyDef.isPublicProp === "function")
            isPublicProp = proxyDef.isPublicProp;
        else if (Array.isArray(publicPropList))
            isPublicProp = _isPublicPropByList.bind(this, publicPropList);
        ...
        if (typeof this._obj[proxyHandlerSymbols.set] === "function") {
            defaultProxyDef.set = (target, property, value, receiver) => {
                if (isPublicMethod(target, property)) {
                    ...
                    exposePublicProperties &&
                    isPublicProp(target, property)
                ) {
                    target[property] = value;
                    return true;
                } else {
                    const r = this._obj[proxyHandlerSymbols.set](
                        property,
                        value
                    );
        ...
        if (
            typeof this._obj[proxyHandlerSymbols.getOwnPropertyDescriptor] ===
            "function"
        ) {
            defaultProxyDef.getOwnPropertyDescriptor = (target, prop) => {
                return this._obj[proxyHandlerSymbols.getOwnPropertyDescriptor](
                    target,
                    prop
                );
            };
        ...
        if (
            typeof this._obj[proxyHandlerSymbols.deleteProperty] === "function"
        ) {
        ...
        if (typeof this._obj[proxyHandlerSymbols.apply] === "function") {
            defaultProxyDef.apply = (target, thisArg, argumentsList) => {
                return this._obj[proxyHandlerSymbols.apply](
                    target,
                    thisArg,
                    argumentsList
                );
        ...
        if (typeof this._obj[proxyHandlerSymbols.construct] === "function") {
            defaultProxyDef.apply = (target, argumentsList, newTarget) => {
                return this._obj[proxyHandlerSymbols.construct](
                    target,
                    argumentsList,
                    newTarget
                );
        ...
        if (
            typeof this._obj[proxyHandlerSymbols.defineProperty] === "function"
        ) {
            defaultProxyDef.apply = (target, property, descriptor) => {
                return this._obj[proxyHandlerSymbols.defineProperty](
                    target,
                    property,
                    descriptor
                );
        ...
        if (
            typeof this._obj[proxyHandlerSymbols.getPrototypeOf] === "function"
        ) {
        ...
        if (
            typeof this._obj[proxyHandlerSymbols.preventExtensions] ===
            "function"
        ) {
            defaultProxyDef.apply = target => {
        ...
        if (
            typeof this._obj[proxyHandlerSymbols.setPrototypeOf] === "function"
        ) {
            defaultProxyDef.apply = (target, prototype) => {
                return this._obj[proxyHandlerSymbols.setPrototypeOf](
                    target,
                    prototype
                );
            };
        ...
        if (!Object.keys(defaultProxyDef).length)
            throw new Error(
                "Cannot create empty proxy. No proxy handler symbols can be found!"
            );
